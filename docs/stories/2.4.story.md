...
- [x] **Task 1: Create Protocol Synthesizer Agent** (AC: 1, 2)
  - [x] Create `agents/protocolSynthesizer.ts`
  - [x] Configure with Claude Sonnet 4 model
  - [x] Write synthesis system prompt
  - [x] Define structured output schema for protocols
  - [x] Handle multi-step extraction

- [x] **Task 2: Create Synthesis Prompt** (AC: 2)
  - [x] Design prompt to extract: tasks, sequences, warnings, hacks
  - [x] Include examples of expected output format
  - [x] Add instructions for dependency detection
  - [x] Include attribution extraction rules

- [x] **Task 3: Implement Dependency Graph** (AC: 3, 4)
  - [x] Create `lib/graph/dependencyGraph.ts`
  - [x] Parse sequence signals from synthesis output
  - [x] Build directed acyclic graph (DAG)
  - [x] Implement topological sort for ordering
  - [x] Handle circular dependency detection

- [x] **Task 4: Create Attribution Scorer** (AC: 5)
  - [x] Create `lib/scoring/attribution.ts`
  - [x] Score by: recency, engagement (upvotes), specificity
  - [x] Select best attribution per protocol step
  - [x] Handle multiple sources per step

- [x] **Task 5: Create Synthesis Action** (AC: 6)
  - [x] Create `convex/ai/synthesis.ts`
  - [x] Implement `synthesizeProtocols` action
  - [x] Fetch ingested content for corridor
  - [x] Call protocol synthesizer agent
  - [x] Order steps and attach attributions
  - [x] Save to protocols table via batchCreateProtocols

- [x] **Task 6: Implement Multilingual Synthesis** (AC: 7)
  - [x] Accept `targetLanguage` parameter
  - [x] Add translation instructions to prompt
  - [x] Verify output in target language
  - [x] Handle language codes: en, yo, hi, pt, tl

- [x] **Task 7: Create Full Pipeline Action** (AC: 1-7)
  - [x] Create `convex/ai/pipeline.ts`
  - [x] Implement `generateCorridorProtocols` action
  - [x] Chain: research → synthesis → storage
  - [x] Handle errors at each stage
  - [x] Return protocol IDs on success
...
- Fixed implicit any types in pipeline.ts by adding explicit type annotations
- Fixed Map iterator compatibility by using Array.from() for ES5 target
- Fixed action self-reference calls by using ctx.runAction instead of direct calls

### Completion Notes List
- Protocol Synthesizer Agent created with comprehensive extraction rules
- Dependency Graph implements DAG with topological sort and cycle detection
- Attribution Scorer ranks sources by engagement, recency, and quality
- Synthesis Action parses agent output and validates protocol data
- Full Pipeline chains research → synthesis → storage with error handling
- Multilingual support via targetLanguage parameter in prompts

### File List
- agents/protocolSynthesizer.ts - Protocol synthesis agent with Claude Sonnet 4
- lib/graph/dependencyGraph.ts - DAG builder and topological sort
- lib/scoring/attribution.ts - Attribution scoring and selection
- convex/ai/synthesis.ts - Synthesis action and re-synthesis
- convex/ai/pipeline.ts - Full pipeline orchestration
