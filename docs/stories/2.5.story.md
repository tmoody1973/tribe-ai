...
- [x] **Task 1: Add Freshness Fields to Corridors** (AC: 2, 5)
  - [x] Update `convex/schema.ts` corridors table:
...
- [x] **Task 2: Create Freshness Check Query** (AC: 1, 2)
  - [x] Create `convex/corridors.ts` → `checkFreshness` query
  - [x] Calculate staleness: `Date.now() - lastResearchedAt > 30 days`
  - [x] Return: `{ isFresh, isStale, lastResearched, status }`
  - [x] Handle corridors with no research yet

- [x] **Task 3: Create Cache-First Protocol Retrieval** (AC: 1)
  - [x] Create `convex/protocols.ts` → `getProtocolsWithFreshness` query
  - [x] Check corridor freshness
  - [x] Return protocols with freshness metadata
  - [x] Include `stale: boolean` flag in response

- [x] **Task 4: Implement Background Refresh** (AC: 3)
  - [x] Create `convex/ai/refresh.ts` → `refreshCorridorInBackground` action
  - [x] Set status to "refreshing" before starting
  - [x] Run research and synthesis pipeline
  - [x] Update status to "fresh" on success, "error" on failure
  - [x] Use Convex scheduler for background execution

- [x] **Task 5: Create Stale Protocol Handler** (AC: 3)
  - [x] Update `getProtocolsWithFreshness` to trigger refresh
  - [x] Return cached data immediately
  - [x] Schedule background refresh via scheduler
  - [x] Don't block on refresh completion

- [x] **Task 6: Create Admin Refresh Function** (AC: 4)
  - [x] Create `convex/admin.ts` → `forceRefreshCorridor` action
  - [x] Accept corridorId parameter
  - [x] Bypass freshness check
  - [x] Trigger immediate research + synthesis
  - [x] Useful for testing and manual intervention

- [x] **Task 7: Implement Cache Metrics Logging** (AC: 6)
  - [x] Create `convex/metrics.ts`
  - [x] Log: cache hits, cache misses, refresh triggers
  - [x] Store in metrics table for dashboard
  - [x] Add timestamp and corridorId context

- [x] **Task 8: Create Scheduled Cleanup Job** (AC: 2)
  - [x] Create `convex/crons.ts` with scheduled function
  - [x] Run daily: find stale corridors
  - [x] Pre-emptively refresh popular corridors
  - [x] Clean up expired ingested content
...
- Added freshness tracking fields to corridors schema (lastResearchedAt, researchStatus, protocolCount, errorMessage)
- Added metrics table for cache hit/miss tracking
- Created freshness constants library with threshold calculations
- Implemented checkFreshness query for corridor status
- Created getProtocolsWithFreshness for cache-first retrieval
- Implemented getProtocolsAndRefreshIfStale action with scheduler integration
- Created refreshCorridorInBackground action for async pipeline execution
- Added forceRefreshCorridor admin action for manual testing
- Implemented metrics logging (cache_hit, cache_miss, corridor_refreshed, corridor_refresh_error)
- Created cron jobs for daily cleanup and stale corridor refresh
- Added maintenance functions for expired content and metrics cleanup

### File List
- convex/schema.ts - Updated corridors table, added metrics table
- lib/constants/freshness.ts - Freshness threshold constants
- convex/corridors.ts - Added checkFreshness, updateResearchStatus, getStaleCorridors
- convex/protocols.ts - Added getProtocolsWithFreshness, getProtocolsAndRefreshIfStale, deleteByCorridorId
- convex/ai/refresh.ts - Background refresh action
- convex/admin.ts - Force refresh admin action
- convex/metrics.ts - Metrics logging and queries
- convex/crons.ts - Scheduled cleanup and refresh jobs
- convex/maintenance.ts - Cleanup functions for content and metrics
