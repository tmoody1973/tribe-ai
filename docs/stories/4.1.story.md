...
- [x] **Task 1: Install CopilotKit Dependencies** (AC: 1)
  - [x] Install `@copilotkit/react-core`
  - [x] Install `@copilotkit/react-ui`
  - [x] Verify peer dependencies resolved (from Story 3.5)

- [x] **Task 2: Create CopilotKit Provider** (AC: 1, 3, 6)
  - [x] Create `components/providers/CopilotProvider.tsx`
  - [x] Configure runtime URL pointing to API route
  - [x] Pass corridor, stage, language as properties
  - [x] Wrap dashboard layout with provider

- [x] **Task 3: Create Chat Window Component** (AC: 2, 4, 7)
  - [x] Create `components/chat/ChatWindow.tsx`
  - [x] Configure CopilotChat with custom labels
  - [x] Add initial welcome message
  - [x] Apply RetroUI styling overrides

- [x] **Task 4: Implement State Sync** (AC: 6)
  - [x] Use `useCopilotReadable` for corridor data
  - [x] Sync protocol status to agent context
  - [x] Update context on data changes

- [x] **Task 5: Create Chat History Persistence** (AC: 5)
  - [x] Add `chatMessages` table to Convex schema
  - [x] Create `saveMessage` mutation
  - [x] Create `getMessages` query
  - [x] Create `clearHistory` mutation

- [x] **Task 6: Create Chat API Route** (AC: 1, 4)
  - [x] Create `app/api/copilotkit/route.ts`
  - [x] Configure GoogleGenerativeAIAdapter with gemini-3-flash-preview
  - [x] Enable streaming responses

- [x] **Task 7: Style Chat with RetroUI** (AC: 7)
  - [x] Override CopilotKit CSS variables in globals.css
  - [x] Apply neobrutalist border and shadow styles
  - [x] Match input styling to theme
  - [x] Style message bubbles

- [x] **Task 8: Add Chat to Dashboard** (AC: 1-7)
  - [x] Add chat route `app/[locale]/(dashboard)/chat/page.tsx`
  - [x] Add chat translations to en.json
  - [x] Handle loading and no-corridor states
...
### Files Created
- `components/chat/ChatWindow.tsx` - Chat window component with CopilotKit integration
- `app/[locale]/(dashboard)/chat/page.tsx` - Chat page route
- `convex/chat.ts` - Chat message persistence (saveMessage, getMessages, clearHistory)

### Files Modified
- `components/providers/CopilotProvider.tsx` - Enhanced with corridor/stage/language context
- `convex/schema.ts` - Added chatMessages table
- `messages/en.json` - Added chat translations
- `app/globals.css` - Added CopilotKit RetroUI theme overrides

### Implementation Notes
- CopilotProvider passes corridor, stage, and language to CopilotKit properties
- ChatWindow uses useCopilotReadable for bidirectional state sync
- Chat history persisted to Convex with user/corridor indexes
- RetroUI styling applied via CSS custom properties and attribute selectors
- Gemini 3 Flash Preview used as LLM backend (from Story 3.5)

### Build Status
- TypeScript: ✅ Passed
- Next.js build: ✅ Passed
- Chat route: /[locale]/chat (288 kB bundle)
