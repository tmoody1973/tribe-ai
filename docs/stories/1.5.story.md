...
- Extended Convex schema with originCountry, destinationCountry, stage, visaType, updatedAt fields
- Updated completeOnboarding mutation to accept onboarding data
- Added updateProfile mutation for settings page
- Created migration stages constants (dreaming, planning, preparing, relocating, settling)
- Created countries data file with 89 countries and flag emojis
- Created OnboardingWizard multi-step form with progress indicator
- Created CountrySelector with searchable dropdown
- Created StageSelector with localized stage cards
- Created onboarding page with redirect logic for completed users
- Created DashboardGuard component for onboarding redirect
- Created useRequireOnboarding hook for reusable logic
- Created settings page with editable profile fields
- Added onboarding and settings translations to all 9 locale files
- Build verified successful

### File List
**Created:**
- `apps/web/lib/constants/stages.ts` - Migration stages configuration
- `apps/web/lib/constants/countries.ts` - Country data with flags
- `apps/web/components/onboarding/CountrySelector.tsx` - Searchable country dropdown
- `apps/web/components/onboarding/StageSelector.tsx` - Migration stage selector
- `apps/web/components/onboarding/ProgressIndicator.tsx` - Step progress indicator
- `apps/web/components/onboarding/OnboardingWizard.tsx` - Multi-step onboarding form
- `apps/web/components/layout/DashboardGuard.tsx` - Onboarding redirect wrapper
- `apps/web/hooks/useRequireOnboarding.ts` - Onboarding check hook
- `apps/web/app/[locale]/onboarding/page.tsx` - Onboarding page
- `apps/web/app/[locale]/settings/page.tsx` - Profile settings page

**Modified:**
- `apps/web/convex/schema.ts` - Added onboarding fields to users table
- `apps/web/convex/users.ts` - Updated completeOnboarding, added updateProfile
- `apps/web/app/[locale]/(dashboard)/layout.tsx` - Added DashboardGuard
- `apps/web/messages/*.json` - All 9 translation files with onboarding/settings keys
