    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      try {
        ...
        for (const analysis of analyses) {
          ...
          // Filter threshold: only include relevance >= 30 (lowered for better content discovery)
        ...
        log(`Batch ${batchIndex + 1}: Analyzed ${analyses.length} posts, kept ${analyses.filter(a => a.relevanceScore >= 30).length} relevant`);
      } catch (err) {
